...
import { Eye, Edit, Trash2, Mail, Key, MoreVertical, Download, Plus, Search, Sliders } from 'lucide-react';
    ...
    <div className="page-users">
      <div className="page-header users-page-header">
        <div>
          ...
          <p className="users-page-subtitle">Centralized user operations, access status, and lifecycle management.</p>
        </div>
      ...
      <div className="users-insight-row mb-3">
        ...
        <div className="users-insight users-insight-inactive">
          ...
          <span className="users-insight-value">{users.filter(u => u.status === 'inactive').length}</span>
      ...
      <div className="card users-list-card">
        <div className="users-toolbar">
          <div className="users-toolbar-left">
            <div className="users-filter-tabs">
              ...
              >
                Active <span className="users-filter-count">{users.filter(u => u.status === 'active').length}</span>
              </button>
              <button 
                className={`users-filter-tab ${filterStatus === 'pending' ? 'active' : ''}`}
                onClick={() => setFilterStatus('pending')}
              >
                Pending <span className="users-filter-count">{users.filter(u => u.status === 'pending').length}</span>
              </button>
              <button 
                className={`users-filter-tab ${filterStatus === 'inactive' ? 'active' : ''}`}
                onClick={() => setFilterStatus('inactive')}
              >
                Inactive <span className="users-filter-count">{users.filter(u => u.status === 'inactive').length}</span>
              </button>
            </div>
          </div>
                  ...
                  <tr key={user.id}>
                    ...
                    <td>
                      <div className="users-user">
                        <div className="users-avatar-wrap">
                          <img src={`assets/img/avatars/${user.avatar}`} alt="" className="users-avatar" />
                          <span className={`users-avatar-status ${user.status}`}></span>
                        </div>
                        <div className="users-user-info">
                          <Link to={`/users/${user.id}`} className="users-user-name">{user.name}</Link>
                    ...
                    <td>
                      <div className="users-actions">
                        <Link to={`/users/${user.id}`} className="users-action-btn" title="View">
                          <Eye size={16} />
                        </Link>
                        <Link to={`/users/${user.id}/edit`} className="users-action-btn" title="Edit">
                          <Edit size={16} />
                        </Link>
                        <div className="dropdown">
                          ...
                          <ul className="dropdown-menu dropdown-menu-end">
                            <li><a className="dropdown-item" href="#"><Mail className="inline mr-2" size={14} /> Send Email</a></li>
                            <li><a className="dropdown-item" href="#"><Key className="inline mr-2" size={14} /> Reset Password</a></li>
                            ...
                            <li>
                              ...
                              >
                                <Trash2 className="inline mr-2" size={14} /> Delete
                              </button>
                            </li>
                          </ul>
                        </div>
        ...
        <div className="users-pagination">
          <div className="users-pagination-info">Showing <strong>1-{filteredUsers.length}</strong> of <strong>{users.length}</strong> users</div>
          <nav>
