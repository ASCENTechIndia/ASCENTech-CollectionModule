...
async function createCompanyRepo(payload) {
  const statement = `
    BEGIN
      AOUP_COMPANYMST_INS_UPD(
        ...
        :in_num_companymst_parentid,
        :in_num_companymst_brcategory,
        :in_var_companymst_ishobranch,
        :in_var_companymst_address,
        :in_num_companymst_mobileno,
        :in_var_companymst_email,
        :in_var_companymst_remark,
        :in_blob_companymst_logo,
        :in_var_companymst_insby,
        :in_num_companymst_upassoperid,
        :in_var_companymst_branchcode,
        :in_var_companymst_legalname,
        :in_var_companymst_companytype,
        :in_var_companymst_status,
        :in_var_companymst_pan,
        :in_var_companymst_gst,
        :in_var_companymst_cin,
        :in_clob_companymst_config,
        :in_username,
        :out_ErrorCode,
        :out_ErrorMsg
      );
  ...
  const binds = {
  in_num_companymst_compid: Number(payload.compid),

  in_var_companymst_compname: payload.compname,

  in_var_companymst_branchname: normalizeNullable(payload.branchname),

  in_num_companymst_parentid: normalizeNullable(payload.parentid),

  in_num_companymst_brcategory: normalizeNullable(payload.brcategory),

  in_var_companymst_ishobranch: payload.ishobranch || "N",

  in_var_companymst_address: normalizeNullable(payload.address),

  in_num_companymst_mobileno: normalizeNullable(payload.mobileno),

  in_var_companymst_email: normalizeNullable(payload.email),

  in_var_companymst_remark: normalizeNullable(payload.remark),

  // BLOB
  in_blob_companymst_logo: {
    dir: oracledb.BIND_IN,
    type: oracledb.BLOB,
    val: payload.logo || null,
  },

  in_var_companymst_insby: normalizeNullable(payload.insby),

  in_num_companymst_upassoperid: normalizeNullable(payload.upassoperid),

  in_var_companymst_branchcode: normalizeNullable(payload.branchcode),

  in_var_companymst_legalname: normalizeNullable(payload.legalname),

  in_var_companymst_companytype: normalizeNullable(payload.companytype),

  in_var_companymst_status: payload.status || "ACTIVE",

  in_var_companymst_pan: normalizeNullable(payload.pan),

  in_var_companymst_gst: normalizeNullable(payload.gst),

  in_var_companymst_cin: normalizeNullable(payload.cin),
  ...
  console.log("Company Bind Payload:", binds);
