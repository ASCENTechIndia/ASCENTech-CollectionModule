...
async function getCompanyRepo() {
  const query = `select num_companymst_compid as compid ,var_companymst_compname as comp_name, var_companymst_branchname as comp_branch from etech_cm.aoup_companymst_def order by var_companymst_branchname`;
  return executeQuery(query);
}

async function getAgencyRepo() {
  const query = `select num_agencymst_id as agency_id, var_agencymst_name as agency_name from etech_cm.aoup_agencymst_def `;
  return executeQuery(query);
}
