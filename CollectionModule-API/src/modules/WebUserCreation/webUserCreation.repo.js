...
/**
 * Get dropdown options for web user creation form
 */
...
/**
 * Get branches for specific branch category (when user is at branch level)
 */
async function getBranchByIdRepo(branchId) {
  const query = `SELECT branchname as name, brid as id, branchcode as code FROM branchlist WHERE brid = ${branchId} ORDER BY branchname`;
  return executeQuery(query);
}

/**
 * Get roles for web user creation (role-based filtering)
 * For Branch/Category 6: only roles 2 and 5
 * For other categories: all roles
 */
async function getRolesWebRepo(branchCategory) {
  let whereClause = '';
  
  // Branch category 6 = Branch level, only show roles 2 and 5
  if (branchCategory === 6 || branchCategory === '6') {
    whereClause = `WHERE num_userrole_id IN (2, 5)`;
  }
  
  const query = `SELECT var_userrole_name as name, num_userrole_id as id FROM etech_cm.aoup_userrole_mas ${whereClause} ORDER BY num_userrole_id`;
  return executeQuery(query);
}

/**
 * Get user details by user ID for web editing
 */
...
/**
 * Update web user using stored procedure
 */
...
/**
 * Upload web user image blob
 */
async function uploadWebUserImageRepo(userId, imageData, imageType, imagePosition = 1) {
  const statement = `
    BEGIN
      UPDATE aoup_usermst_def
      SET ${imagePosition === 1 ? 'blob_usermst_proofimage, var_usermst_imagetype' : 'blob_usermst_proofimage2, var_usermst_imagetype2'} = :imageData, :imageType
      WHERE var_usermst_userid = :userId;
    END;
  `;
 ...
 * Get employer list
 */
async function getEmployerListRepo() {
  const query = `SELECT var_employer_name as name, num_employer_id as id, var_employer_code as code FROM aoup_employer_mas ORDER BY var_employer_name`;
  return executeQuery(query);
}
